...
class FluxPhase2hBoundaryTest {
 @Test fun `debug exposes decoder verification release does not and Generate stays disabled`() {val debug=File("src/debug/java/com/google/ai/edge/gallery/customtasks/flux/FluxDeveloperVerificationSection.kt").readText();val release=File("src/release/java/com/google/ai/edge/gallery/customtasks/flux/FluxDeveloperVerificationSection.kt").readText();assertContains(debug,"Verify VAE Decoder and Image");assertFalse(release.contains("Verify VAE Decoder and Image"));val screen=File("src/main/java/com/google/ai/edge/gallery/customtasks/flux/FluxEditorScreen.kt").readText();assertContains(screen,"enabled = false")}
