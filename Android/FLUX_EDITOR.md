...
Interpretation is deterministic and limited to: `PROMPT_INFLUENCE_OBSERVED`, `PROMPT_DIFFERENCE_LOST_BEFORE_TEXT_CONDITIONING`, `PROMPT_DIFFERENCE_LOST_DURING_TRANSFORMER`, `FINAL_BITMAPS_NEARLY_IDENTICAL`, `COMPARISON_INCONCLUSIVE`, `CANCELLED`, or `ERROR`. Sanitized summaries include backend `GPU FP32`, device/API, fixed seed, graph sequence, completion/failure stage, checkpoint metrics, per-run durations, Java heap and process PSS before/after, thermal status before/after, and cancellation availability. They exclude prompt text, selected URI, provider, filename, filesystem path, image bytes, tensor values, credentials, Hugging Face token, and model cache path.
